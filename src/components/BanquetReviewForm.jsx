...
import { User, Mail, Calendar as CalendarIcon, Phone, PartyPopper, Users, Sparkles, Coins, Star, MessageCircle, Send, X } from 'lucide-react';
...
const BanquetReviewForm = ({ onBack, onClose, onSuccess, viewOnly = false, initialValues = {} }) => {
      ...
      <div className="bg-white rounded-2xl p-0 w-full max-w-2xl shadow-xl relative max-h-[95vh] overflow-y-auto border border-gray-200">
        ...
        <div className="flex flex-col items-center pt-10 pb-2 mt-8">
          <div className="bg-purple-100 rounded-full w-16 h-16 flex items-center justify-center mb-4">
            <PartyPopper className="text-purple-500 h-8 w-8" />
          </div>
          <h2 className="text-3xl font-bold text-center mb-1">Banquet Review</h2>
          {viewOnly && (
            <div className="text-center text-yellow-700 text-base font-semibold mb-2">(View Only)</div>
          )}
          <p className="text-center text-gray-600 mb-4">Share your event experience with us</p>
        </div>
        {/* Card */}
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm mx-6 mb-8 p-0">
          ...
          <FormProvider {...methods}>
            <form onSubmit={handleSubmit(onSubmit)} className="px-8 pb-8 pt-0 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
              ...
              {/* Event Date */}
              <div className="col-span-1">
                <FormLabel className="flex items-center mb-1"><CalendarIcon className="mr-2 h-4 w-4" /> Event Date <span className="text-red-500 ml-1">*</span></FormLabel>
                <input
                  {...register('visit_date')}
                  type="text"
                  placeholder="dd-mm-yyyy"
              ...
              <div className="col-span-2 mt-8">
                ...
                <textarea
                  ...
                  placeholder="Please share your detailed feedback about your event experience..."
              ...
              {!viewOnly && (
                <div className="col-span-2 flex justify-end mt-8">
                  <button
                    ...
                    disabled={submitting}
                  >
                    <Send className="mr-2 h-5 w-5" /> Submit Banquet Review
                  </button>
                </div>
              )}
            </form>
...
export default BanquetReviewForm; 